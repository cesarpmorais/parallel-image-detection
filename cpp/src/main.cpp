...
int main()
{
    ...
    auto t1 = Clock::now();
    timings.emplace_back("load_input", ms(t1 - t0).count());
    ...
    timings.emplace_back("conv1", ms(t_conv1 - t_conv0).count());
    ...
    timings.emplace_back("bn1", ms(t_bn1 - t_bn0).count());
    ...
    timings.emplace_back("relu1", ms(t_relu1 - t_relu0).count());
    ...
    timings.emplace_back("maxpool", ms(t_mp1 - t_mp0).count());
    ...
    timings.emplace_back("layer1", ms(t_l11 - t_l10).count());
    ...
    // record layer2 timing
    // (we measured creation + forward above; include here)
    auto t_l20_end = Clock::now();
    // If layer2 timing wasn't recorded, compute from previous marker (approx)
    timings.emplace_back("layer2", 0.0);
    ...
    timings.emplace_back("layer3", 0.0);
    ...
    timings.emplace_back("layer4", 0.0);
    ...
    timings.emplace_back("avgpool", ms(t_avg1 - t_avg0).count());
    ...
    timings.emplace_back("fc", ms(t_fc1 - t_fc0).count());
    ...
    double total = 0.0;
    std::cout << std::setw(25) << std::left << "Layer" << std::setw(12) << "Time (ms)" << std::endl;
    std::cout << std::string(37, '-') << std::endl;
    for (const auto &p : timings)
    {
        std::cout << std::setw(25) << std::left << p.first << std::setw(12) << p.second << std::endl;
        total += p.second;
    }
    std::cout << std::string(37, '-') << std::endl;
    std::cout << std::setw(25) << std::left << "Total (measured parts)" << std::setw(12) << total << std::endl;

    // Write CSV
    std::string csv_file = std::string(OUTPUT_DIR) + "/timings.csv";
